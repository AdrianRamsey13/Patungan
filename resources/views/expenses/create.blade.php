...
<div class="max-w-lg mx-auto px-4 py-6">
    ...
    <div class="flex items-center gap-2.5 bg-white rounded-[14px] px-4 py-3 mb-5"
        ...
        <div>
            ...
            <div class="text-muted text-xs font-semibold">{{ $event->members()->count() }} anggota</div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('events.expenses.store', $event) }}"
        ...
        <div class="bg-white rounded-[22px] p-6 flex flex-col gap-5"
            ...
            {{-- Kamu sebagai payer --}}
            <div class="flex items-center gap-3 bg-coral-soft rounded-[14px] px-4 py-3">
                <x-pt.avatar :name="Auth::user()->name" size="md" ring="#FFE7DF" />
                <div>
                    <div class="text-xs font-bold text-coral">Kamu yang nalangin</div>
                    <div class="font-extrabold text-ink text-sm">{{ Auth::user()->name }}</div>
                </div>
            </div>
            ...
            <div class="rounded-[14px] bg-cream px-4 py-3">
                <div class="flex items-center justify-between">
                    <span class="text-ink-soft text-sm font-semibold">
                        Split rata ke {{ $event->members()->count() }} orang
                    </span>
                ...
                <div class="mt-3 flex items-center gap-2 flex-wrap">
                    @foreach ($event->members as $member)
                        <div class="flex items-center gap-1.5 bg-white rounded-pill px-2 py-1"
                            ...
                            <x-pt.avatar :name="$member->name" size="xs" />
                            <span class="text-ink text-xs font-bold">
                                {{ $member->id === Auth::id() ? 'Kamu' : explode(' ', $member->name)[0] }}
                            </span>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="mt-4 bg-white rounded-[18px] px-5 py-4"
             style="box-shadow:0 -4px 16px rgba(36,29,22,.06)">
            <button type="submit"
                    :disabled="totalNum() <= 0"
                    class="pt-btn pt-btn-primary w-full justify-center rounded-[14px] py-4 text-base font-bold"
                    :class="totalNum() <= 0 ? 'opacity-50 cursor-not-allowed' : ''">
